    ...
    <div className="pt-[72px]" data-testid="gallery-page">
      <section className="relative overflow-hidden border-b border-white/10 bg-black">
        ...
        <div className="relative mx-auto max-w-[1440px] px-5 pb-16 pt-20 sm:px-8 sm:pt-28">
          <SectionHead index="GAL" eyebrow="From The Shop Floor" title="The Gallery" dark />
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {PHOTOS.map((p, i) => (
            <Reveal key={p.label} delay={i * 0.08} className={p.big ? "lg:col-span-2" : ""}>
              <figure
                className="corner-frame spotlight-card group relative h-full overflow-hidden border border-line bg-black"
              ...
              >
                ...
                />
                <figcaption className="absolute inset-x-0 bottom-0 flex items-center justify-between border-t border-white/15 bg-black/85 px-5 py-3 backdrop-blur">
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-forge">{p.label}</p>
                    <p className="mt-0.5 font-mono text-[10px] uppercase tracking-[0.2em] text-white/50">{p.sub}</p>
                  </div>
